...
| 用途 | 単位 | 備考 |
|------|------|------|
| `font-size` | `rem` | ルート基準。`px` / `em` は使わない |
| `line-height` | 単位なし（倍率） | `1.6` など |
| レイアウトの余白・幅（section 間、container など） | `px` / `%` | 画面サイズに対する絶対的な指定 |
| コンポーネント内部の余白・幅 | `px` / `rem` | ルートフォントサイズと連動させたい場合は `rem` |
| テキストに追随すべきサイズ | `em` | アイコン・インジケーターなど、当該要素の `font-size` と明示的に連動させたい場合のみ |
| ブレークポイント | `px` | `min-width` で指定 |

> `em` は cascade により意図せず値が変わりやすいため、**テキスト連動が明確に必要な場合のみ**使用する。
